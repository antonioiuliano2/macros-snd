...
bool CheckNeutrinoVertexPosition(TVector3 nu_vertex){
  ...
  const Double32_t targetxmin = -47.6; 
  const Double32_t targetxmax = -8.0;
  const Double32_t targetymin = 15.5;
  const Double32_t targetymax = 55.1;
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH2D *hvxy_intarget = new TH2D("hvxy_intarget","Position of neutrino interaction vertices in the transverse plane;x[cm];y[cm]", 100,-100,0,100,0,100);
