...
const int brick = 22; //3 with large angles
const int firstplate = 5;
const int lastplate = 6;
const int plates[6] = {7, 12, 18, 19, 20, 21};
const int nplates = 6;
...
void linkreports(){
    ...
    for (int iplate = 0; iplate < nplates; iplate++){
        ...
        inputfile = TFile::Open(Form((path+"/b%06i/p%03d/%i.%i.0.0.cp.root").Data(),brick,plates[iplate],brick,plates[iplate]));
	if (inputfile){
 	 if (inputfile->Get("report")){
          ...
          c->Draw();
          c->Print(Form((path+"/b%06i/plots/link_reports/link_p%i.png").Data(),brick,plates[iplate]),"png");
         }
         else cout<<"coul not open report for plate "<<plates[iplate]<<endl;        
         //close the file
         inputfile->Close();
	}
    }
...
void drawallthicknesses(){
    ...
    for (int i = 0; i < nplates; i++){
        TFile *f = TFile::Open(Form((path+"/b%06d/p%03d/%i.%i.0.0.raw.root").Data(),brick,plates[i],brick,plates[i]));
        if (!f) continue;
        if (!f->Get("Views")){ cout<<"Not found tree Views, raw file probably corrupted, skipping"<<endl; continue;}
        thickness();
        gROOT->GetSelectedPad()->GetCanvas()->SetName(Form("canvas%d",plates[i]));
        ...
        cout<<"Plate number: "<<plates[i]<<endl;
        ...
        canvas->Print(Form((path+"/b%06d/plots/thicknesses/thickness_plate%i.png").Data(),brick,plates[i]));
...
void drawallraws(){
  ...
  for (int i = 0; i < nplates; i++){
	...
	TCanvas *cview = (TCanvas*) gROOT->FindObject("view");
        ...
        cz->Print(Form((path+"/b%06d/plots/raws/checkz_plate%i.png").Data(),brick,plates[i]));
	cview->Print(Form((path+"/b%06d/plots/raws/checkview_plate%i.png").Data(),brick,plates[i]));
	csurf->Print(Form((path+"/b%06d/plots/raws/checksurf_plate%i.png").Data(),brick,plates[i]));
