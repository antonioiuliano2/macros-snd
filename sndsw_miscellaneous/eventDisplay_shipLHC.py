...
# check for end vertex
 ...
 def DrawMCTracks(self,option=''):
  ...
  for fT in sTree.MCTrack:
   ...
   #adding condition on level to draw
   maxlevelnudaughters = 2 #we only try to draw tracks related to neutrino events
   level = 0
   checktrack = False
   mytrack = fT
   while (level < maxlevelnudaughters):
    mothertrackID = mytrack.GetMotherId()
    if (mothertrackID >= 0):
     mytrack = sTree.MCTrack[mothertrackID]
     if (mothertrackID == 0): #we arrived to neutrino, accept the track
      checktrack = True
    level = level + 1

   if not checktrack: 
    continue #we do not draw this track
  ...
  # loop over all sensitive volumes to find hits
   for P in ["vetoPoint","EmulsionDetPoint","ScifiPoint", "MuFilterPoint"]:
